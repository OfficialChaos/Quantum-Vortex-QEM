"""
run_hardware_v2.py — IBM Quantum Hardware Validation (Exp 2)
Quantum-Vortex QEM Project  |  Shawn G. Kleipe  |  v0.4.0

PURPOSE
-------
Experiment 2: validates ε hierarchy across QAOA and Random circuits.
Paired with Exp 1 (test+vqe), this completes all 4 circuit types on
real hardware, directly testing the paper's claim VQE>QAOA>random>test.

DESIGN
------
  Circuits : random (random single-qubit+CNOT), qaoa (ZZ+Rx mixer)
...
def make_random_circuit(depth: int) -> QuantumCircuit:
    """Randomised single-qubit gates + CNOT — matches paper Random circuit.
    Uses fixed seed for reproducibility across trials.
    Gates sampled from {Rx, Ry, Rz} with random angles in [0, 2π].
    """
...
def make_qaoa_circuit(depth: int) -> QuantumCircuit:
    """QAOA-style circuit: alternating ZZ problem-unitary + Rx mixer.
    Matches paper QAOA circuit exactly.
    Uses fixed gamma=0.3, beta=0.2 angles for reproducibility.
    """
    gamma = 0.3   # problem unitary angle
    beta  = 0.2   # mixer angle
    qc    = QuantumCircuit(2)
    for _ in range(depth):
        # ZZ problem unitary: exp(-i gamma Z0Z1)
        qc.cx(0, 1)
        qc.rz(2 * gamma, 1)
        qc.cx(0, 1)
        # Rx mixer
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Quantum-Vortex QEM — Hardware Validation v2 (QAOA + Random)")
    print(f"  {timestamp}")
    print(f"{'='*60}\n")
    ...
    sim_ref = {
        ...
        ('random','fibonacci',4): (-0.192, 0.275, 0.00997),
        ('random','linear',   4): (-0.068, 0.447, 0.01583),
        ('qaoa',  'fibonacci',4): (+0.001, 0.374, 0.01249),
        ('qaoa',  'linear',   4): (-0.234, 0.547, 0.02190),
    }
    ...
    save_data = {
        'metadata': {
            ...
            'paper_version': 'v0.4.0', 'experiment': 2,
        },
        'results': output,
    }
    fname = f"hardware_results_v2_{timestamp[:10]}.json"
