...
\newcommand{\GITHUBURL}{https://github.com/YOUR\_USERNAME/Quantum-Vortex-QEM}
\newcommand{\ZENODOID}{ZENODO\_DOI\_PENDING}
...
Our results show [FILL AFTER EXPERIMENTS].
...
[FILL AFTER EXPERIMENTS — paste summary table here]
...
\begin{table}[h]
\centering
\caption{ZNE performance at depth $d=4$, noise $p=0.01$, $N=1000$ shots,
10 trials. $\varepsilon$: mean energy norm. Flag\%: fraction of trials
flagged by stability filter ($\Gamma=0.05$).}
\label{tab:results}
\begin{tabular}{lcccc}
\toprule
Schedule & ZNE Mean & ZNE Std & $\varepsilon$ Mean & Flag\% \\
\midrule
Fibonacci & X.XXX & X.XXX & X.XXX & X\% \\
Linear    & X.XXX & X.XXX & X.XXX & X\% \\
Odd       & X.XXX & X.XXX & X.XXX & X\% \\
\bottomrule
\end{tabular}
\end{table}
...
[DESCRIBE FINDINGS AFTER EXPERIMENTS]
...
Experimental results across [X] configurations demonstrate [FILL].
