...
Our results across 1,620 experiment runs (3 circuit types $\times$ 3 schedules $\times$ 3 extrapolants $\times$ 6 depths $\times$ 6 noise levels $\times$ 5 trials) show that the stability filter correctly identifies reliable ZNE regimes across all circuit types. VQE circuits exhibit systematically higher energy norms than test or random circuits, revealing circuit-type dependence not previously characterized. Richardson extrapolation produces the highest variance across all configurations, while the odd schedule with polynomial extrapolation achieves the lowest variance on VQE circuits ($\sigma = 0.024$).
...
trials to estimate variance across 1,620 total experiment runs.
...
and $N=200$ shots per evaluation averaged over 5 trials, covering
3 circuit types, 3 schedules, and 3 extrapolants (1,620 total runs).
...
\begin{table}[!ht]
\centering
\caption{ZNE performance at $d=4$, $p=0.01$, linear extrapolant, 5 trials.
...
\midrule
Test   & Fibonacci & $-0.020 \pm 0.021$ & 0.00547 \\
Test   & Linear    & $+0.013 \pm 0.039$ & 0.01005 \\
Test   & Odd       & $-0.037 \pm 0.032$ & 0.00345 \\
\midrule
VQE    & Fibonacci & $-0.609 \pm 0.030$ & 0.01977 \\
VQE    & Linear    & $-0.629 \pm 0.047$ & 0.01295 \\
VQE    & Odd       & $-0.615 \pm 0.023$ & 0.01811 \\
\midrule
Random & Fibonacci & $+0.465 \pm 0.017$ & 0.01371 \\
Random & Linear    & $+0.536 \pm 0.032$ & 0.01382 \\
Random & Odd       & $+0.482 \pm 0.056$ & 0.01395 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Fibonacci & 0.030 & 0.153 & 0.055 \\
Linear    & 0.047 & 0.104 & 0.093 \\
Odd       & 0.023 & 0.070 & 0.024 \\
\bottomrule
\end{tabular}
\end{table}
...
extrapolation is highly sensitive to shot noise. Polynomial (degree-2)
extrapolation with the odd schedule achieves the lowest variance
on VQE circuits ($\sigma = 0.024$), suggesting this combination
as the most stable for hardware-efficient ansatz circuits.
...
achieves the lowest $\varepsilon$ (0.00345). On VQE circuits,
Fibonacci produces the highest $\varepsilon$ (0.01977) while
linear produces the lowest variance — suggesting that Fibonacci's
wider scale factor range (reaching 13 vs.\ 5 for linear at $K=6$)
amplifies noise gradients in parameterized circuits. On random
circuits, all three schedules converge to nearly identical
$\varepsilon$ values (0.0137--0.0140), indicating the filter is
...
\begin{figure}[!ht]
    ...
    Dashed line: threshold $\Gamma=0.05$. Error bars: std over 5 trials.}
    \label{fig:stability}
\end{figure}
...
At $p = 0.1$, $d = 12$, singular regions were identified for all
...
Experimental results across 1,620 runs demonstrate that the stability filter reliably identifies well-conditioned ZNE regimes across three circuit types. VQE circuits show systematically higher energy norms than structured or random circuits — a previously uncharacterized circuit-type dependence. Richardson extrapolation consistently produces the highest variance, while odd scheduling with polynomial extrapolation achieves the lowest variance on VQE circuits.
