"""
run_hardware_v3.py — IBM Quantum Hardware Validation (Exp 3)
Quantum-Vortex QEM Project  |  Shawn G. Kleipe  |  v0.4.0

PURPOSE
-------
Experiment 3: tests filter at d=6 and d=8 — the flag boundary.
Key test: does the TEST circuit show the period-8 ε spike on hardware?
If yes: confirms the physical resonance is real, not simulation artifact.

DESIGN
------
  Circuits : test (Rx(π/4)+CNOT), vqe (Ry(π/4)+CNOT, i.e. CX — native IBM gate)
  Depths   : d=6, d=8  (flag boundary — tests filter sensitivity)
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"  Quantum-Vortex QEM — Hardware Validation v3 (d=6,8 depth sweep)")
    print(f"  {timestamp}")
    print(f"{'='*60}\n")
    ...
    sim_ref = {
        ...
        ('test','fibonacci',6): (-0.007, 0.033, 0.00666),
        ('test','fibonacci',8): (-0.542, 0.018, 0.04781),
        ('test','linear',6): (+0.005, 0.052, 0.00782),
        ('test','linear',8): (-0.718, 0.026, 0.05568),  #  ← EXPECT FLAG
        ('vqe','fibonacci',6): (-0.118, 0.401, 0.01800),
        ('vqe','fibonacci',8): (-0.164, 0.325, 0.02299),
        ('vqe','linear',6): (+0.095, 0.459, 0.02364),
        ('vqe','linear',8): (-0.267, 0.343, 0.02511),
    }
    ...
    save_data = {
        'metadata': {
            ...
            'paper_version': 'v0.4.0', 'experiment': 3,
        },
        'results': output,
    }
    fname = f"hardware_results_v3_{timestamp[:10]}.json"
